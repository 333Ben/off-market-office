...
- **Contact via Max (real)** — build a contact list in the table, then hand it to **Max** (Digital Crew's AI sales agent). Enabled with `PROVIDERS=max` + `DIGITALCREW_API_TOKEN` (a `max_live_` key) in `.env`. `POST /api/outreach/launch {companyIds, channel}` calls Max's real REST API (`https://max.digitalcrew.tech`, `POST /api/v1/prospect-lists/import-csv`) to create a **real prospect list in the Max workspace**, then marks the companies *contacted*. Running an actual send campaign additionally needs a sending account connected in the Max app (Unipile); until then the list is created and the response says so. Any failure falls back to the mock, so the demo never breaks. Enrichment also returns **phone and LinkedIn** alongside email.
