...
- **Contact via Max (real)** — build a contact list in the table, then hand it to **Max** (Digital Crew's AI sales agent). Enabled with `PROVIDERS=max` + `DIGITALCREW_API_TOKEN` (a `max_live_` key) in `.env`. `POST /api/outreach/launch {companyIds, channel}` calls Max's real REST API (`https://max.digitalcrew.tech`, `POST /api/v1/prospect-lists/import-csv`) to create a **real prospect list in the Max workspace**, then marks the companies *contacted*. If a sending account is connected in the **same Max workspace as the key**, it goes further — AI-generates a workflow (`POST /api/v1/ai-agent/generate-workflow`), creates the campaign, and launches it (`POST /api/v1/campaigns/{id}/launch`); otherwise it stops at the list and says a sending account is needed (set `MAX_DRAFT_ONLY=1` to create the campaign without sending). Any failure falls back to the mock, so the demo never breaks. Enrichment also returns **phone and LinkedIn** alongside email.
